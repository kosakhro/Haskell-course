...
drawWorld (Play rocks (Ship (x,y) (vx,vy)) (Ufo (ux,uy) (uvx, uvy)) bullets)
  = pictures [ship, asteroids, ufo, shots]
   where
    ship      = color red (pictures [translate x y s (polygon (asteroidShape x y s))])
...
shipShape :: Float -> Float -> Float -> [Point]
shipShape x y s = [(x,y+s),(x+s,y), (x,y+0.5),(x+0.4*s,y-0.4*s),(x-0.4*s,y+0.5*s),(x-0.2*s,y-0.2*s),(x+0.6*s,y+0.3*s)]
